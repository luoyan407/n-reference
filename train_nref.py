...
parser.add_argument('--ref_gt_dir', default='', 
                    type=str,
                    help='reference human fixation maps path')
parser.add_argument('--num_ref_samples', default=0, type=int,
                    help='referral samples')
...
parser.add_argument('--ref_fxt_size', nargs='+', type=int,
                    help='resized reference fixation resolution: (600, 800) | (480, 640) | (320, 640)')
parser.add_argument('--out_dir', default='logs/salicon_dinet_websal', 
                    type=str,
                    help='output folder for logs, saliency maps, snapshots, and etc')
...
parser.add_argument('--ref_threshold', default=0.0, type=float,
                    help='reference threshold')
parser.add_argument('--ref_lr', default=1.0, type=float,
                    help='learning rate used in the reference process')
parser.add_argument('--ref_lr_coef', default=1.0, type=float,
                    help='learning rate coefficient used in the reference process')
parser.add_argument('--ref_lamb', default=0.01, type=float,
                    help='regularization weight used in the reference process')
parser.add_argument('--ref_batch_size', default=1, type=int,
                    help='reference sample batch size')
...
output = model_body(oneimage)

train_loader, val_loader, ref_loader, print_str = create_nshot_loaders(args,
...
number_of_params_body = sum(p.numel() for p in model.parameters())
number_of_params_head = sum(p.numel() for p in model_head.parameters())
...
print('===Total parameters number: {}+{}'.format(number_of_params_body, number_of_params_head))
print('===Total parameters number: {}+{}'.format(number_of_params_body, number_of_params_head), file=txtlogger, flush=True)
...
for epoch in range(args.start_epoch, args.epochs):
    ...
    adjust_learning_rate(args.lr, optimizer_head, epoch, 
       basenum=args.lr_decay_epoch, coef=args.lr_coef)
    model_head.lr = args.ref_lr * args.ref_lr_coef ** int(epoch/args.lr_decay_epoch)
    cur_lr = optimizer.param_groups[0]['lr']

    sal_paths = ['{}/ep{}'.format(args.sal_path, epoch+1)]
    train_loss, val_loss, train_batchtime, train_datatime, val_batchtime, val_datatime = train_val_ref(
                                    model, model_head, criterion, optimizer, optimizer_head, epoch,
                                    train_loader, val_loader, ref_loader, sal_paths, txtlogger)
    ...
    save_filename = os.path.join(snapshot_dir, 'model_head_ep{epoch}.pth'.format(epoch=epoch+1))
    save_checkpoint(model_head, save_filename)
...
if args.eval_mode == 'asynchronous':
    for epoch in range(args.start_epoch, args.epochs):
        ...
        evalCmd = 'python src/eval_command.py --output "{}" --fixation_folder "{}" --salmap_folder "{}" --appendix "{}" --notify "{}"'.format(
            stat_file, args.val_gt_dir, sal_path, appendix, isnotify)
        sproc = subprocess.Popen(evalCmd, shell=True)
        sproc.wait()